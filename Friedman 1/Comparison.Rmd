---
title: "Friedman 1 Dataset (D1): Comparison"
...
### Friedman 1
```{r}
gendata.friedman1=function(n,p=100,seed=NULL,rho=NULL,sd=1){
  set.seed(seed)
  if(is.null(rho)){
    rho=diag(p)
  }
  ...
  L=chol(rho)
  x=u%*%L
  y=10*sin(x[,1]*x[,2]*pi)+20*(x[,3]-0.5)^2+10*x[,4]+5*x[,5]+rnorm(n,sd=sd)
  data <- data.frame(x = x, y = y)
...
```{r}
train_data=gendata.friedman1(n,seed=42,p=p,sd=sqrt(5))
SDy=sd(train_data$y)
...
#Generate test data
test_data = gendata.friedman1(1e4, seed = 41,sd=sqrt(5))
...
ggplot(coefficients_df, aes(coeff,fill = type,order=type)) +
  ...
  theme(legend.text=element_text(size=15),
        axis.text=element_text(size=15),strip.text.y = element_text(size=15),
...
Delta=dist(apply(matrix(shapley_df$val,nrow=n),2,function(x){x/(sd(x)^0.5)}))
HC_fit=hclust(Delta,method = 'average')
points_order=order(HC_fit$order)
shapley_df$order=points_order

#Plot
ggplot(shapley_df[shapley_df$predictor %in% 1:10,], aes(x = order, y = factor(paste0('x.',predictor),levels=paste0('x.',10:1)), fill = val)) +
  scale_fill_gradient2(low='blue',high='red',mid='white',midpoint = 0, guide=guide_colourbar(title="Shapley value", label.position='left',title.position = 'right', title.hjust = 0.6)) +
  geom_tile()+
  theme_classic()+
...
OG_values=c(10*sin(x[,1]*x[,2]*pi),
            10*sin(x[,1]*x[,2]*pi),
            20*(x[,3]-0.5)^2,
            10*x[,4],
            5*x[,5])/data_info$sd_y
...
#Plot the results
ggplot(loc_imps[loc_imps$predictor <= 8 & loc_imps$predictor >= 3,], aes(x = x, y = value, color = type)) +
  geom_point(size=0.5) +
  geom_line()+
  facet_grid(factor(paste0('x.',predictor),levels=paste0('x.',3:8)) ~ ., scales = "fixed") +
...
Predictors 1 and 2 together:

```{r}
combi_shapleys_df=rbind(data.frame(
  value=loc_imps$value[loc_imps$predictor == 1 & loc_imps$type == 'Friedman-Popescu, sign-sensitive']+loc_imps$value[loc_imps$predictor == 2 & loc_imps$type == 'Friedman-Popescu, sign-sensitive'],type='Friedman-Popescu, sign-sensitive', true=loc_imps$value[loc_imps$type=='True contribution' & loc_imps$predictor==1]),
  data.frame(
  value=loc_imps$value[loc_imps$predictor == 1 & loc_imps$type == 'Friedman-Popescu, + absolute value']+loc_imps$value[loc_imps$predictor == 2 & loc_imps$type == 'Friedman-Popescu, + absolute value'],type='Friedman-Popescu, absolute value', true=loc_imps$value[loc_imps$type=='True contribution' & loc_imps$predictor==1]),
  data.frame(
  value=loc_imps$value[loc_imps$predictor == 1 & loc_imps$type == 'Shapley']+loc_imps$value[loc_imps$predictor == 2 & loc_imps$type == 'Shapley'],type='Shapley',
  true=loc_imps$value[loc_imps$type=='True contribution' & loc_imps$predictor==1]))
...
imp_measures$benchmark=c(rep(integrate(Vectorize(function(x){abs(integrate(function(z){10*sin(x*z*pi)},lower=0,upper=1)$value-integrate(Vectorize(function(x){integrate(function(z){10*sin(x*z*pi)},lower=0,upper=1)$value}),lower=0,upper=1)$value)}),lower=0,upper=1)$value, 2), #pred 1 and 2
                         integrate(function(x){abs(20*(x-0.5)^2-5/3)},lower=0,upper=1)$value, #pred 3
                         integrate(function(x){abs(10*x-5)},lower=0,upper=1)$value, #pred 4
                         integrate(function(x){abs(5*x-5/2)},lower=0,upper=1)$value, #pred 5
                         rep(0,p-5))/data_info$sd_y #everything else. also, divide by SDy
